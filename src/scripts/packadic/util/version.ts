...
function parse(version?:any, loose?:any) {
...
function valid(version, loose) {
    var v:any = parse(version, loose);
    return v ? v.version : null;
}

function clean(version, loose) {
    var s:any = parse(version.trim().replace(/^[=v]+/, ''), loose);
...
export class SemVer {
    ...
    public compareMain(other:any):any {
        if (!(other instanceof SemVer))
            other = new SemVer(other, this.loose);
    ...
    public comparePre(other:any):any {
        if (!(other instanceof SemVer))
            other = new SemVer(other, this.loose);
    ...
    public inc(release:any, identifier:any):any {
        switch (release) {
            case 'premajor':
...
function diff(version1:any, version2:any) {
    if (eq(version1, version2)) {
        return null;
    } else {
        var v1:any = parse(version1);
        var v2:any = parse(version2);
...
function eq(a:any, b:any, loose?:any) {
    return compare(a, b, loose) === 0;
}
...
export class VersionRange {
    ...
    public parseRange(range:any):any {
