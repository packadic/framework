module packadic {
    ...
    export class ConfigObject implements IConfig {
        ...
        public unset(prop:any):any {
            prop    = prop.split('.');
            var key = prop.pop();
            var obj = util.objectGet(this.data, ConfigObject.getPropString(prop.join('.')));
            delete obj[key];
        }
        ...
        public raw(prop?:any):any {
            if (prop) {
                return util.objectGet(this.data, ConfigObject.getPropString(prop));
            } else {
                return this.data;
            }
        }
        ...
        public set(prop:string, value:any):ConfigObject {
            util.objectSet(this.data, ConfigObject.getPropString(prop), value);
            return this;
        }
